...
Can someone steal my passwords just by being on the same coffee shop Wi‑Fi?

Yes, if the websites you visit use unencrypted connections (HTTP instead of HTTPS) or if your session cookies are transmitted without the Secure flag. A VPN prevents this by encrypting all your traffic before it leaves your device.

### Does HTTPS protect me on public Wi‑Fi?

Partially. HTTPS encrypts the content of your communication with the website, but it does not hide which websites you visit (DNS and SNI are visible), does not protect against evil twin networks, and can be bypassed by SSL stripping if your browser's HTTPS-Only Mode is not enabled.

What is the difference between packet sniffing and a man-in-the-middle attack?

Packet sniffing is passive — the attacker listens to data traveling across the network without interfering. A man-in-the-middle attack is active — the attacker intercepts and potentially modifies the communication between your device and the server.

### Can a VPN protect me from an evil twin attack?
...
### Is it safe to use mobile data instead of public Wi‑Fi?
...
*   Public Wi-Fi networks allow attackers to intercept traffic using Evil Twin access points, ARP spoofing, and SSL stripping
*   HTTPS alone does not protect against DNS spoofing, SNI snooping, or captive portal credential harvesting
*   Juice jacking and malicious USB charging stations can compromise mobile devices without Wi-Fi connection
*   In-tunnel DNS and full-tunnel post-quantum encryption ensure zero unencrypted traffic leaves your device
*   Always enable an automatic kill switch to prevent data leaks if the Wi-Fi connection drops

## Related reading

*   [PRIVACYFree VPN — the honest 2026 guide8 min · OllaVPN Blog](/free-vpn)
*   [SECURITYFree VPN for Public Wi-Fi: Complete Traveler Defense7 min · OllaVPN Blog](/blog/free-vpn-for-public-wifi-guide.html)
*   [SECURITYWhat is a DNS leak? How to test, fix, and prevent it9 min · OllaVPN Blog](/blog/what-is-a-dns-leak.html)
*   [PRIVACYHow Advertisers Track You Across the Internet10 min · OllaVPN Blog](/blog/how-advertisers-track-you-across-the-internet.html)
...
Read guide →](what-logs-does-vpn-keep.html)