...
1.  [1\. Why this matters more than people think](#why-this-matters-more-than-people-think)
2.  [2\. What an ISP actually is, technically](#what-an-isp-actually-is-technically)
3.  [3\. The full list: everything your ISP can see](#the-full-list-everything-your-isp-can-see)
4.  [4\. What HTTPS hides — and what it doesn’t](#what-https-hides-and-what-it-doesn-t)
5.  [5\. The DNS problem, explained simply](#the-dns-problem-explained-simply)
6.  [6\. What your ISP cannot see with a VPN](#what-your-isp-cannot-see-with-a-vpn)
7.  [7\. A short history of ISP data privacy law](#a-short-history-of-isp-data-privacy-law)
8.  [8\. Why ISPs collect this data in the first place](#why-isps-collect-this-data-in-the-first-place)
9.  [9\. Data retention laws around the world](#data-retention-laws-around-the-world)
10.  [10\. Five common misconceptions](#five-common-misconceptions)
11.  [11\. How to actually hide your activity from your ISP](#how-to-actually-hide-your-activity-from-your-isp)
12.  [12\. VPN vs. encrypted DNS vs. Tor](#vpn-vs-encrypted-dns-vs-tor)
13.  [13\. Your ISP-privacy checklist](#your-isp-privacy-checklist)
14.  [14\. How OllaVPN thinks about ISP visibility](#how-ollavpn-thinks-about-isp-visibility)
...
Pillar guide: What is a DNS leak? How to test, fix, and prevent it https://ollavpn.com/blog/what-is-a-dns-leak.html

Pillar guide: What is post-quantum cryptography? A 2026 guide https://ollavpn.com/blog/what-is-post-quantum-cryptography.html

Pillar guide: Five Eyes, Nine Eyes, and Fourteen Eyes explained https://ollavpn.com/blog/five-eyes-nine-eyes-fourteen-eyes.html

What is Encrypted Client Hello? https://ollavpn.com/blog/what-is-encrypted-client-hello.html

Can governments track VPNs? https://ollavpn.com/blog/can-government-track-vpn.html

Why are free VPNs free? https://ollavpn.com/blog/why-are-free-vpns-free.html

Best free VPN for Windows in 2026 https://ollavpn.com/blog/best-free-vpn-for-windows.html

A broader-context closing thought

There’s a pattern we keep running into across every topic we cover on this blog, and ISP visibility is one of the cleanest examples of it: the gap between what people assume protects them and what actually does is widest at exactly the points where the underlying technology is most invisible. Nobody sees a DNS lookup happen. Nobody sees the SNI field pass by in plain text. The entire mechanism this guide describes is invisible by design—which is precisely why “HTTPS means I’m private” persists as a belief long after it stopped being fully true.

The remedy, as with the rest of our writing, is depth over speed: explain the actual mechanism, name the actual protocol, and give a way to verify the claim rather than asking the reader to trust it. We’d rather this guide be 5,000 words that leave you able to check your own setup than 500 words that leave you with a vague sense of unease and a coupon code.

If you’ve read this far, you’re the reader we’re writing for. We update guides like this one whenever something material changes—a protocol reaches broader support, a law changes, a carrier gets caught doing something new with subscriber data. The main free-VPN comparison, the technology overview, and the free privacy tools are the three pages most worth bookmarking if this guide was useful to you.

Verify and cross-reference

Two quick ways to check any claim on this page.

Verify from your own device: our free DNS lookup, WebRTC leak test, and what-is-my-IP tools need no account and run in your browser—connect to whatever VPN you use and run them, and in under five minutes you’ll know whether DNS is actually leaking.

Cross-reference an independent source: the Electronic Frontier Foundation’s technical explainers on HTTPS and encrypted DNS, and the IETF’s own RFCs (8484 for DoH, 7858 for DoT), are strong non-commercial starting points, since we're an operator with a commercial interest in the conclusion. And read your own ISP’s actual privacy policy once—it’s the single most specific primary source available to you.

What to actually do about it

In priority order: turn on a VPN that handles DNS correctly today, even on the free tier—our best free VPN 2026 guide compares trustworthy options beyond our own product. Run a leak test on whatever you’re already using—plenty of people who believe they’re protected are quietly leaking DNS. Check your router and smart-home settings once, since a VPN on your phone or laptop alone doesn’t touch that layer. And reassess periodically—new devices join your network and standards like ECH keep rolling out, so a quarterly check is a reasonable habit.

About this guide

If you spot a fact that’s drifted or a mechanism we should add, email

[\[email protected\]](/cdn-cgi/l/email-protection). Reader corrections drive a meaningful share of our quarterly updates.

⭐ Key takeaways

What to remember from this guide

Why this matters more than people think What an ISP actually is, technically The full list: everything your ISP can see What HTTPS hides — and what it doesn’t The DNS problem, explained simply Frequently asked questions

Can my ISP see what websites I visit even if I use HTTPS?

Yes, in most default configurations. HTTPS encrypts the content of a page, but the domain name typically leaks through two channels: the DNS lookup that happens before the connection, and the unencrypted SNI field in the TLS handshake itself. Encrypted DNS and Encrypted Client Hello (ECH) are the specific fixes for each leak, respectively.

Does my ISP know what I search for on Google?

The search query text itself is encrypted on virtually all major search engines today. But the fact that you visited the search engine, and roughly when, is still visible through the same DNS/SNI mechanism described above, unless that traffic is routed through a VPN with in-tunnel DNS.

Can my ISP sell my browsing data?

In the United States, generally yes, since the 2017 repeal of the FCC’s 2016 broadband privacy rules removed the requirement for opt-in consent before ISPs use or sell browsing-adjacent data. Rules vary by country and, within the US, by state—California, Virginia, and several other states have their own consumer privacy laws that provide some opt-out rights.

Does incognito or private browsing mode hide my activity from my ISP?

No. Private browsing only controls what’s stored locally on your device. It has no effect on what leaves your device over the network—your ISP sees identical DNS lookups and traffic patterns whether or not you’re browsing privately.

Does a VPN completely hide me from my ISP?

It hides the overwhelming majority of what’s described in this guide—destination domains, DNS lookups, and content—provided the VPN handles DNS inside its encrypted tunnel without leaking. It does not hide the fact that you’re using a VPN, and it shifts the trust question to your VPN provider instead of your ISP, which is why a VPN’s [no-logs](/blog/which-free-vpn-does-not-keep-logs.html) policy matters as much as its encryption.

What is the SNI leak, and why does it matter?

Server Name Indication (SNI) is a field in the TLS handshake that tells a server which site’s certificate to present, and on most connections today it travels unencrypted even though the rest of the exchange is encrypted. It means your ISP can see the domain you’re connecting to on an HTTPS site without breaking any encryption at all. Encrypted Client Hello (ECH) is the newer standard designed specifically to close this leak.

Can my ISP see what I do inside apps, not just websites?

To a meaningful degree, yes, through the same metadata mechanisms. The domains and IP addresses an app connects to, timing, and data volume are all visible in the same way they are for browser traffic, even though the ISP generally can’t see inside an app’s encrypted content.

Is it legal for ISPs to track this data?

Largely yes, within limits that vary by country. Operational metadata collection for billing, security, and network management is standard and legal essentially everywhere. Whether an ISP can use or sell that data for advertising without consent depends heavily on jurisdiction—the EU’s GDPR requires strong consent protections, while the US, absent the repealed 2016 FCC rules, has a more permissive federal baseline with state-level exceptions.

Does changing my DNS server stop my ISP from tracking me?

Only partially, and only if the new DNS connection is encrypted. Pointing your device at a third-party DNS resolver changes who answers the lookup, but unless it’s done over DNS over HTTPS or DNS over TLS, the query can still be visible in transit to your ISP’s network equipment.

Does OllaVPN keep logs of what my ISP would otherwise see?

No. OllaVPN runs a [no-logs](/blog/which-free-vpn-does-not-keep-logs.html) policy and handles DNS entirely inside the encrypted tunnel by default, so there’s essentially nothing retained about your browsing activity—by your ISP or by us—under this configuration. Combined with an always-on kill switch and post-quantum cryptography, that’s the practical technical posture that closes the gaps described throughout this guide.