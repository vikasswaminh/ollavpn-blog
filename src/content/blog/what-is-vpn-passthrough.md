...
Now, when you use a VPN, you're essentially creating a secret, [encrypted tunnel](/blog/what-is-a-vpn-tunnel.html) directly from your apartment (device) to a different, secure building far away. This tunnel is a [VPN tunnel](/what-is-a-vpn-tunnel), and it's a special kind of traffic. Without VPN passthrough, your router (the doorman) might not understand this special tunnel traffic. It might block it or mishandle it because it's trying to apply its usual NAT rules, which aren't designed for an encrypted tunnel that's already doing its own thing.
