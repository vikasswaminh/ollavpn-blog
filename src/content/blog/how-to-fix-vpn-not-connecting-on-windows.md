...
**Step 5: Change the [VPN protocol](/blog/what-is-a-vpn-protocol.html).** OllaVPN uses WireGuard by default for its speed and security, but sometimes certain network configurations might interfere with specific protocols. In your OllaVPN app settings, look for an option to change the VPN protocol. Try switching to [QUIC](/features/quic-protocol) if it's available, as it's designed to be more resilient on restrictive networks. If that doesn't work, you might try other common protocols like OpenVPN or IKEv2 if your client offers them, though OllaVPN primarily focuses on WireGuard and QUIC for optimal performance and future-proofing.

**Step 6: Update your OllaVPN app.** Make sure you're running the latest version of the OllaVPN application. Developers frequently release updates that fix bugs and improve connection reliability. Check for updates within the app or download the newest version from the [OllaVPN website](/download).
