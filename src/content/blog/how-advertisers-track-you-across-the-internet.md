...
What is [post-quantum cryptography](/blog/what-is-post-quantum-cryptography.html)? A 2026 guide
...
The category evolves. Ad tracking didn’t arrive fully formed - it changed as browsers, mobile operating systems, and regulations changed. Third-party cookies were a result of stateless HTTP meeting a business need. Fingerprinting grew as cookies got blocked. Server-side tracking grew as fingerprinting protection improved. If you understand that evolution, you can better anticipate what comes next - and it’s usually more escalation on both sides.

Standards bodies matter too. Privacy-related work in groups like W3C and IETF shapes what the web platform allows and what gets standardized. That influences the surveillance landscape over the next few years.

Finally, the threat model matters. “Advertisers tracking you” can range from mildly annoying to materially harmful. Know what level of risk matters for you, and build your toolkit accordingly.

## About this guide

Maintained by Nathan Pratt, OllaVPN's Privacy & Security Lead. Fact-checked by Hannah Wu, Senior Security Engineer. We refresh this post quarterly to reflect changes in tracking methods, browser behavior, and regulatory landscape. The last full evaluation was completed July 2026; the next is scheduled for October 2026.

If you spot a fact that’s drifted or a tracking method we haven’t covered, email [\[email protected\]](/cdn-cgi/l/email-protection). Reader corrections drive a meaningful percentage of our quarterly updates.

## Key takeaways

What to remember from this guide

Advertising tracking uses dozens of simultaneous methods - cookies, fingerprinting, pixels, IP tracking, login identity, and more - not just one

Clearing cookies doesn’t protect against fingerprinting, login-based tracking, or IP-based profiling

Incognito/private browsing mode protects your local history only - websites still track you normally

The “death of cookies” has been replaced by fingerprinting, CNAME cloaking, and server-side tracking - not by privacy

A VPN handles IP masking and ISP-level surveillance; it belongs in a toolkit alongside a privacy browser and content blocker

Data brokers hold profiles including inferred health conditions, political views, and financial stress - and sell them to insurers, employers, and law enforcement

## Frequently asked questions

How do advertisers track you without cookies?

Advertisers who can’t use third-party cookies rely primarily on browser fingerprinting, login-based tracking, email hashing, CNAME cloaking, and server-side tracking. Cookies are one method among many, not the whole system.

## Does a VPN stop ad tracking?

A VPN stops two specific tracking vectors: IP address-based tracking and ISP browsing data collection. It does not stop cookie-based tracking, browser fingerprinting, login-based tracking, or tracking pixels. Used alongside a privacy browser and content blocker, it’s a meaningful part of a toolkit. Used alone, it reduces but does not eliminate ad tracking.

## What is browser fingerprinting and how do I stop it?

Browser fingerprinting identifies you by combining your browser type, screen resolution, installed fonts, GPU rendering characteristics, timezone, and dozens of other technical attributes into a unique identifier. To reduce it: use Firefox with fingerprint protection enabled, or Brave (which randomizes fingerprint attributes per session). Avoid Chrome if ad tracking is a concern. Tor Browser provides stronger fingerprint protection by standardizing many attributes, but at the cost of speed and compatibility.

## Are tracking pixels illegal?

Tracking pixels are legal in most jurisdictions for general advertising. In the EU, GDPR requires consent for non-essential tracking pixels. In the US, the FTC and various state laws create some restrictions, particularly in sensitive sectors. For most consumer websites, tracking pixels remain legal and common.

## What do data brokers know about me?

Data brokers typically hold your name, address history, phone numbers, email addresses, purchase history, location history (often GPS-precise from app data), inferred interests and demographics, political and charitable donation history (from public records), and inferred health and financial status. Major brokers like Acxiom and Oracle claim files on hundreds of millions of people. Under California and EU law, you can request your data from most major brokers and ask for deletion.

## What’s the best browser for privacy from ad tracking?

For most users, Firefox with Enhanced Tracking Protection set to “Strict” mode, combined with uBlock Origin, is the most practical high-privacy setup. Brave is an alternative that goes further on fingerprint randomization but is less established than Firefox. Safari on Apple devices has strong ITP but is closed source. Tor Browser provides the strongest theoretical protection but is impractical for everyday use at scale. Avoid Chrome if ad tracking is a concern.

## How does login-based tracking work?

When you’re logged into Google, Meta, or another major platform, those companies recognize you as a specific, identified user rather than an anonymous browser. Because their tracking code runs on a large percentage of websites, they see your activity on those sites while logged in and tie it to your real account. This is deterministic tracking. It’s more accurate than cookie or fingerprint tracking and can’t be blocked with browser settings. The countermeasure is logging out while browsing, or using a dedicated browser profile without logged-in accounts.

## Does OllaVPN log my browsing data?

No. OllaVPN operates a [zero-logs](/blog/what-logs-does-vpn-keep.html) architecture - it does not record connection timestamps, IP addresses, session durations, DNS queries, or any other data that would identify what you did online. DNS queries are resolved inside the encrypted tunnel by OllaVPN’s resolver, so your ISP and third-party DNS providers can’t log them. The architecture has been independently audited. The free plan requires no email address or account, meaning there’s no account identity to associate your traffic with in the first place.

⭐ Key takeaways

## What to remember from this guide

*   Advertisers track you across the web using browser fingerprinting, tracking pixels, CNAME cloaking, and data brokers
*   Incognito mode and clearing cookies do not stop advanced canvas fingerprinting or IP-based identity graph matching
*   Data brokers aggregate offline purchase data, credit card loyalty records, and browsing habits into persistent advertising IDs
*   Combining in-tunnel DNS tracker blocking with a verified zero-logs VPN neutralizes IP tracking and ad network beacons
*   Layered defense—hardened browser settings, disposable aliases, and encrypted tunnels—provides the highest privacy protection

## Related reading

*   [PRIVACYFree VPN — the honest 2026 guide8 min · OllaVPN Blog](/free-vpn)
*   [SECURITYHow Public Wi-Fi Steals Passwords (And How to Stop It)9 min · OllaVPN Blog](/blog/how-public-wifi-steals-passwords.html)
...
#### More in [Privacy guides](index.html#privacy-security) 26 guides

[View all 26 in Privacy guides →](index.html#privacy-security)

[

How Public Wi‑Fi Can Steal Your Passwords (2026 Guide)

Read guide →](how-public-wifi-steals-passwords.html)[

Five Eyes, Nine Eyes, and Fourteen Eyes Explained (2026 Guide)

Read guide →](five-eyes-nine-eyes-fourteen-eyes.html)[

What Is a DNS Leak? How to Test, Fix, and Prevent It (2026 Guide)

Read guide →](what-is-a-dns-leak.html)[

What Is a VPN Kill Switch and Why Is It Critical? (2026 Guide)

Read guide →](what-is-a-vpn-kill-switch.html)