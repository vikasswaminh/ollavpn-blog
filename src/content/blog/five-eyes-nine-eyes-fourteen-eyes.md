...
### Does OllaVPN keep logs that could be shared under any of these alliances?

**No.** OllaVPN runs a no-logs policy and is designed so that there is essentially nothing to hand over under any government data request, regardless of jurisdiction. Combined with [post-quantum cryptography](what-is-post-quantum-cryptography.html) and an always-on kill switch, that's the practical posture that matters more than which country the company is registered in.
